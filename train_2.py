...
# Custom init functions for TE attention/MLP blocks (following https://arxiv.org/pdf/2310.17813)
def te_init_method(weight):
    """Init for QKV and FC1 weights in TE blocks."""
...
def te_output_layer_init_method(weight):
    """Init for PROJ and FC2 weights in TE blocks - zeroed out."""
...
class TransformerBlock(nn.Module):
    ...
    def forward(
        ...
        B, T, _ = qkv.shape
        qkv = qkv.view(B, T, 3, n_head, self.head_dim)
        q, k, v = qkv.unbind(dim=2)
        q = self.q_norm(q)
        k = self.k_norm(k)
        # Apply RoPE to q and k before attention (tensor_format must match qkv_format)
        ...
        attn_out = attn_out.contiguous().view(B, T, n_embd)
...
class LLM(nn.Module):
    ...
    def __init__(self):
        ...
        self.blocks = nn.ModuleDict({f"block_{i}": TransformerBlock(
            layer_number=i + 1
    ...
    def init_weights(self):
        ...
        # TE attention/MLP blocks handle their own init via init_method/output_layer_init_method
...
if USE_FSDP2:
    ...
    # Apply fully_shard to each transformer block first (inner sharding)
